...
void main() {
    ...
    float r = texture(DiffuseSampler, warpedUV + vec2(aberrationX, aberrationY)).r;
    float g = texture(DiffuseSampler, warpedUV).g;
    float b = texture(DiffuseSampler, warpedUV - vec2(aberrationX, aberrationY)).b;
